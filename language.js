(()=>{
  ...
  const KH={
    ...
    'Browse by category':'រកមើលតាមប្រភេទ','Popular Prompts':'Prompt ពេញនិយម','View all prompts →':'មើល Prompt ទាំងអស់ →',
    'High Quality Prompts':'Prompt គុណភាពខ្ពស់','Multiple AI Models':'ម៉ូដែល AI ច្រើន','Instant Access':'ចូលប្រើភ្លាមៗ','Secure & Safe':'សុវត្ថិភាពខ្ពស់',
  ...
  let lang=localStorage.getItem(KEY)==='KH'?'KH':'EN';
  let busy=false;
  ...
  function translateText(root=document.body){
    ...
    const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode:n=>{
      ...
      if(!p||['SCRIPT','STYLE','CODE'].includes(p.tagName)||p.closest('#langSwitch'))return NodeFilter.FILTER_REJECT;
  ...
  function mountLanguage(){
    ...
    box.id='langSwitch';box.className='lang-switch';
    ...
    box.addEventListener('click',e=>{
      ...
      lang=b.dataset.lang==='KH'?'KH':'EN';localStorage.setItem(KEY,lang);translateText(document.body);
      setTimeout(()=>window.NexCredits?.loadBalance?.(),0);
    });
  }
  ...
  function buildNav(){
    ...
    nav.innerHTML=`<a data-nav-view="home" data-view-en="Home" data-view-km="ទំព័រដើម" href="${hrefFor('home')}">Home</a><a data-nav-view="prompt" href="${hrefFor('prompt')}">Prompt</a><a data-nav-view="tools" href="${hrefFor('tools')}">AI Tools</a><a data-nav-view="credits" href="${hrefFor('credits')}">Credits</a><a data-nav-view="how" data-view-en="How it works" data-view-km="របៀបប្រើ" href="${hrefFor('home','#how')}">How it works</a>`;
    ...
    document.querySelectorAll('.brand').forEach(brand=>brand.href=hrefFor('home'));
  }
  ...
  function injectHomeHighlights(){
    ...
    section.innerHTML=`<a class="nex-highlight-card" href="${hrefFor('prompt')}"><div class="nex-highlight-icon">▧</div><h3>Prompt Marketplace</h3><p>Premium image-to-video prompts ready for creators, ads and cinematic content.</p><span class="nex-highlight-link">Browse prompts →</span></a><a class="nex-highlight-card" href="${hrefFor('tools')}"><div class="nex-highlight-icon">✦</div><h3>AI Tools Workspace</h3><p>One member account for AI Chat, Image, Video, Voice and Dubbing tools.</p><span class="nex-highlight-link">Open AI tools →</span></a><a class="nex-highlight-card" href="${hrefFor('credits')}"><div class="nex-highlight-icon">◆</div><h3>Shared Credit Wallet</h3><p>Top up credits with Bakong KHQR and use the same wallet across supported AI tools.</p><span class="nex-highlight-link">Buy credits →</span></a>`;
    hero.insertAdjacentElement('afterend',section);
  }

  function injectPageIntro(view){
    document.getElementById('nexPageIntro')?.remove();if(view==='home')return;
    ...
    const data={prompt:{pill:'PROMPT MARKETPLACE',title:'Image-to-Video Prompt Store',desc:'Browse and buy ready-to-use image-to-video prompts.'},tools:{pill:'MEMBER AI TOOLS',title:'AI Tools Workspace',desc:'Sign in once and use your shared credit wallet across supported AI tools.'},credits:{pill:'AI CREDIT WALLET',title:'Buy AI Credits',desc:'Top up with Bakong KHQR and use credits inside Nex Prompt AI Tools.'}}[view];
    const intro=document.createElement('section');intro.id='nexPageIntro';intro.className='nex-page-intro';
    intro.innerHTML=`<span class="nex-page-pill">${data.pill}</span><h1>${data.title}</h1><p>${data.desc}</p><div class="nex-page-intro-actions"><a href="${hrefFor('home')}">← Home</a>${view==='prompt'?`<a class="primary-link" href="#popular">Browse Prompts</a>`:''}${view==='tools'?`<a class="primary-link" href="${hrefFor('credits')}">Get Credits</a>`:''}${view==='credits'?`<a class="primary-link" href="${hrefFor('tools')}">Open AI Tools</a>`:''}</div>`;
  ...
  function applyView(){
    ...
    if(currentView==='home'){show(['hero','benefits','how','feature','creator','testimonials','faq','contact','footer']);injectHomeHighlights();document.title='Nex Prompt — AI Creator Marketplace'}
    if(currentView==='prompt'){show(['categories','popular','benefits','footer']);injectPageIntro('prompt');document.title='Prompt Marketplace — Nex Prompt'}
    if(currentView==='tools'){show(['tools','footer']);injectPageIntro('tools');document.title='AI Tools — Nex Prompt'}
    if(currentView==='credits'){show(['credits','footer']);injectPageIntro('credits');document.title='AI Credits — Nex Prompt'}
    groups.pricing?.classList.add('nex-view-hidden');groups.models?.classList.add('nex-view-hidden');
  ...
  function wireSearch(){
    ...
    if(currentView==='prompt'){const q=currentUrl.searchParams.get('q');if(q){input.value=q;setTimeout(()=>input.dispatchEvent(new Event('input',{bubbles:true})),150)}}
  }

  function directAuth(mode){
    const tryOpen=()=>{
      if(window.NexCredits?.showAuth){window.NexCredits.showAuth(mode);return true}
      const d=document.getElementById('authDialog');
      if(d){try{if(!d.open)d.showModal()}catch{d.setAttribute('open','')}return true}
      return false;
    };
    if(!tryOpen())setTimeout(tryOpen,80);
  }

  document.addEventListener('click',e=>{
    const signup=e.target.closest?.('#nexSignupTopBtn');
    const signin=e.target.closest?.('#loginBtn');
    if(!signup&&!signin)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
    directAuth(signup?'signup':'login');
  },true);

  window.addEventListener('DOMContentLoaded',()=>{
    addStyles();mountLanguage();buildNav();applyView();wireSearch();translateText(document.body);
    setTimeout(()=>translateText(document.body),700);
